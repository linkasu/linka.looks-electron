...
Сборка осуществляется единой процедурой при помощи Vite, `vite-plugin-electron` и `electron-builder`. Конфигурация TypeScript (`tsconfig.json`) и параметры сборки находятся в корне репозитория. Юнит‑тесты запускаются командой `npm run test:unit`, что помогает отслеживать корректность работы модулей.
