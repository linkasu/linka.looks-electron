...
- `public` — статические файлы, копируемые в renderer build.
...
Файлы конфигурации (`vite.config.ts`, `tsconfig.json` и т.д.) располагаются в корне репозитория. Разработка запускается командой `npm run electron:serve`, итоговая сборка — `npm run electron:build`.
