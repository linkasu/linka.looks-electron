<template>
  ...
  >
    ...
    <div>
      Помогите улучшать LINKa. смотри, разрешив отправку названий использованных функций и версии приложения.
      Содержимое карточек, названия файлов, пути и тексты ошибок не отправляются.
    </div>
    <template #actions>
      <v-btn data-testid="telemetry-privacy" variant="text" @click="openPrivacy">
        Подробнее
      </v-btn>
      <v-btn data-testid="telemetry-defer" variant="text" @click="defer">
        Позже
      </v-btn>
      <v-btn data-testid="telemetry-disable" variant="text" @click="choose('disabled')">
        Не отправлять
      </v-btn>
      <v-btn data-testid="telemetry-enable" color="primary" variant="flat" @click="choose('enabled')">
        Разрешить
      </v-btn>
    </template>
  </v-snackbar>
