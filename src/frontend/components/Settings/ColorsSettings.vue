<template>
  <v-sheet>
    <v-card>
      ...
      <v-card-text>
        <v-text-field v-model="colorPrimary" label="Цвет №1" type="color" />
        <v-text-field v-model="colorAccent" label="Цвет №2" type="color" />
        <v-text-field v-model="colorSecondary" label="Цвет границ" type="color" />
        <v-slider
          v-model="borders"
    ...
    <v-card>
      ...
      <v-card-text>
        <v-row class="mt-1">
          ...
          <v-btn block color="success" class="mt-1" @click="makeBG"> Черно-белая </v-btn>
        </v-row>
      </v-card-text>
    </v-card>
  </v-sheet>
