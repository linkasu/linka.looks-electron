    ...
    <v-row dense>
      <v-col cols="12" lg="8">
        <v-card density="compact">
          <v-card-title class="text-subtitle-1 py-2"> Главные настройки </v-card-title>
          <v-card-text class="py-2">
            <v-row dense>
              <v-col cols="12" sm="6">
                <v-btn block density="comfortable" to="/calibration"> Калибровка смещения </v-btn>
              </v-col>
              <v-col cols="12" sm="6">
                <v-btn
                  block
            ...
            <div class="text-subtitle-2 mb-1">Управление и обратная связь</div>
            <v-row dense>
            ...
            <template v-if="eyeActivation">
              ...
              <div class="d-flex align-center ga-3">
                <div class="text-subtitle-2">Задержка взгляда для активации</div>
                <v-chip color="primary" size="small" variant="tonal"> {{ timeout }} с </v-chip>
              </div>
              <v-slider
                v-model="timeout"
                class="mt-1"
            ...
            <v-divider class="my-2" />
            <div class="text-subtitle-2 mb-1">Конфиденциальность</div>
            <v-radio-group v-model="telemetryConsent" density="compact" hide-details inline>
              <v-radio label="Не отправлять" value="disabled" />
              <v-radio label="Отправлять" value="enabled" />
            </v-radio-group>
            <div class="text-caption text-medium-emphasis">
              Отправляются только названия действий и версия приложения. Содержимое карточек,
              названия файлов, пути и тексты ошибок не отправляются.
            </div>
