...
import { tts } from '../../common/utils/TTSServer'
import { createImageFromText } from '../../common/utils/ImageFromText'
import { appendZip } from '../../common/utils/addToZip'

const DEFAULT_SETS = pathModule.join(__dirname, './../extraResources/defaultSets')
...
export class CardsStorage extends ICloudStorage {
  constructor() {
    ...
    for (const method of methods) {
      ipcMain.handle('storage:' + method.toString(), (_, ...args: any) => {
  ...
  async getFiles(path = ''): Promise<Directory> {
    ...
    const files = (await readdir(dir)).map((f) => pathModule.join(dir, f))
    return files
      .map((file) => {
        try {
  ...
  private checkPath(path: string): string {
    ...
    return normalize(
      path.includes(HOME_DIR) || path.includes(tmpdir())
        ? path
        : join(HOME_DIR, path).replace(/§/g, '/')
    )
  }
  ...
  private getTmpFilename(path: string) {
    return pathModule.join(tmpdir(), pathModule.basename(path))
  }
  ...
  async defaultToTemp(path: string): Promise<string> {
    path = this.checkPath(path)
    const tmp = pathModule.join(tmpdir(), pathModule.basename(path))
  ...
  async moveSet(file: string, location: string) {
    ...
    location = this.checkPath(location)
    const target = pathModule.join(location, pathModule.basename(file))
  ...
  private cleanFile(path: string, config: ConfigFile) {
    ...
    for (const card of config.cards) {
      if (card.cardType === 0) {
        card.audioPath && paths.push(card.audioPath)
        card.imagePath && paths.push(card.imagePath)
      }
    }
  ...
  async addBuffer(path: string, buff: Buffer, ext: string, name?: string) {
    if (!name) name = cryptoModule.randomUUID()
    // const zip = new AdmZip(path)
  ...
  async downloadAndUnpack(url: string): Promise<void> {
    const file = this.getTmpFilename(pathModule.basename(url))
  ...
  private copyFolderSync(srcPath: string, destPath: string) {
    ...
    readdirSync(srcPath).forEach((file: string) => {
      const srcFilePath = pathModule.join(srcPath, file)
      const destFilePath = pathModule.join(destPath, file)
