...
export type ProjectedTelemetry = {
  stream: "product" | "outcome";
  kind: string;
...
const outcomes: Record<OutcomeKind, Readonly<Record<string, readonly string[]>>> = {
  utterance_completed: {
    ...
    mode: ["standard", "direct", "without-space"],
    failure_code: ["engine_unavailable", "request_failed", "timeout", "cancelled"]
  },
  exercise_completed: {
    result: ["completed", "incomplete", "failed"],
    source: ["quiz", "match"],
    count_bucket: ["one", "two_to_five", "six_to_twenty", "more_than_twenty"],
    failure_code: ["state_invalid", "media_unavailable", "interrupted"]
  },
  set_saved: {
  ...
  transfer_completed: {
    ...
    failure_code: ["format_invalid", "media_missing", "storage_failed", "permission_denied"]
  },
  gaze_calibration_completed: {
    result: ["completed", "failed", "cancelled"],
    failure_code: ["device_unavailable", "calibration_failed", "permission_denied"]
  }
};
...
export function projectRendererTelemetry(input: unknown): ProjectedTelemetry | undefined {
  if (!isRecord(input) || Object.keys(input).length !== 1 || typeof input.kind !== "string")
    return undefined;
...
export function projectRendererOutcome(input: unknown): ProjectedTelemetry | undefined {
  if (
    ...
    !Object.prototype.hasOwnProperty.call(outcomes, input.kind)
  )
    return undefined;
